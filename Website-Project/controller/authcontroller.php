...
if (isset($_POST['signup-btn'])) {
    ...
    if ($email !== '') {
        ...
        if ($userCount>0) {
            $errorsS['email'] = 'Email already assigned to an account';
        }
    }
...
//for the admin side of the index page
$animalId = "";
$animalName= "";
$animalType = "";
$animalMI = "";
...
if(isset($_POST['search-btn'])) {
    ...
    if (empty($animalId)) {
        $ERRORS['animal-id'] = "The id field is requiered";
    }
    else{
        $idQuery = "SELECT * FROM animals WHERE id_num=? LIMIT 1";
        ...
        if ($userCount > 0) {
            $ERRORS['animal-id'] = "This id is already assigned";
            $animal = $result->fetch_assoc();
            $animalName = $animal['name'];
            $animalType = $animal['type'];
            if (empty($animal['medical_issue'])) {
                $animalMI = "";
            }
            else{
                $animalMI = "checked";
            }
            $animalPath = $animal['path_image'];
            $animalDescription = $animal['description'];
        }
        else {
            $ERRORS['animal-id'] = "Unassigned Id";
        }
    }
}
...
if (isset($_POST['Update'])) {
    ...
    if (empty($animalName)) {
        $ERRORS['animal-name'] = "The name field is requiered";
    }
    if (empty($animalPath)) {
        $ERRORS['animal-path'] = "The path field is requiered";
    }
    if (empty($animalType)) {
        $ERRORS['animal-type'] = "The type field is requiered";
    }
    elseif ($animalType != "cat" & $animalType != "dog" & $animalType != "other") {
        $ERRORS['animal-type'] = "The type you've inputed isn't one of the choices";
    }
    ...
    if(empty($animalDescription)) {
        $ERRORS['animal-description'] = "The description field is requiered";
    }
    if(count($ERRORS) == 0) {
        $idQuery = "UPDATE animals SET description='$animalDescription', path_image='$animalPath', medical_issue='$animalMI', type='$animalType', name='$animalName' WHERE id_num='$animalId'";
        $stmt = $conn->prepare($idQuery);
        if ($stmt->execute()) {
...
if (isset($_POST['Add'])) {
    ...
    if (count($ERRORS)==0) {
        //checking if the id is not already assigned
        $idQuery = "SELECT * FROM animals WHERE id_num=? LIMIT 1";
        $stmt = $conn->prepare($idQuery);
        $stmt->bind_param('s', $animalId);
        $stmt->execute();
        ...
        //if no error proceed to adding the animal info into the database
        if (count($ERRORS) == 0) {
            $sql = 'INSERT INTO animals (id_num, name, type, medical_issue, path_image, description) VALUES (?, ?, ?, ?, ?, ?)';
            $stmt = $conn->prepare($sql);
            $stmt->bind_param('ssssss', $animalId, $animalName, $animalType, $animalMI, $animalPath, $animalDescription);
            if ($stmt->execute()) {
                $ERRORS['final-message'] = "Successfully added the animal in the database.";
            }
            else{
                $ERRORS['final-message'] = "Database Error: Failed to add to the database";
            }
        }
    }
}
    ...
    if (isset($_POST['Delete'])) {
        ...
        elseif ($userCount>0) {
            $Query = "DELETE FROM animals WHERE id_num='" . $animalId . "'";
            $stmt = $conn->prepare($Query);
            if ($stmt->execute()) {
                $ERRORS['final-message'] = "Successfully deleted the database";
                $animalId = "";
            }
            else {
                $ERRORS['final-message'] = "Failed to connect";
            ...
            $ERRORS['animal-id'] = "The Id is an unassigned Id";
        }
    }